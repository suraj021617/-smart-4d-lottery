...
@app.route('/<path:path>')
def catch_all(path):
    return f"Route /{path} not found. Available routes: /, /pattern-analyzer, /lottery-types, /decision-helper, /quick-pick, etc."
...
def accuracy_tracker_redirect():
    """Redirect to accuracy dashboard"""
    return redirect('/accuracy-dashboard')

@app.route('/pair-analysis')
def pair_analysis():
    """Pair Analysis - Find number pairs that appear together"""
    df = load_csv_data()
    if df.empty:
        return render_template('pair_analysis.html', pairs=[], message="No data available")
    
    # Analyze pairs
    all_nums = []
    for col in ['1st_real', '2nd_real', '3rd_real']:
        all_nums.extend([n for n in df[col].tail(500).astype(str) if n.isdigit() and len(n) == 4])
    
    pair_counts = Counter()
    for i in range(len(all_nums)):
        for j in range(i+1, min(i+10, len(all_nums))):
            pair = tuple(sorted([all_nums[i], all_nums[j]]))
            pair_counts[pair] += 1
    
    pairs = [{'num1': p[0], 'num2': p[1], 'count': c, 'strength': 'Strong' if c >= 5 else 'Medium' if c >= 3 else 'Weak'} 
             for p, c in pair_counts.most_common(50)]
    
    return render_template('pair_analysis.html', pairs=pairs, message="")

@app.route('/time-patterns')
def time_patterns():
    """Time Patterns - Analyze patterns by day/month"""
    df = load_csv_data()
    if df.empty:
        return render_template('time_patterns.html', patterns={}, message="No data available")
    ...
    for _, row in df.tail(1000).iterrows():
        ...
        for col in ['1st_real', '2nd_real', '3rd_real']:
            num = str(row[col])
            if num.isdigit() and len(num) == 4:
                patterns['by_day'][day] = patterns['by_day'].get(day, []) + [num]
                patterns['by_month'][month] = patterns['by_month'].get(month, []) + [num]
                patterns['by_weekday'][weekday] = patterns['by_weekday'].get(weekday, []) + [num]
    
    # Get most common for each
    for key in patterns:
        for subkey in patterns[key]:
            counts = Counter(patterns[key][subkey])
            patterns[key][subkey] = counts.most_common(5)
    
    return render_template('time_patterns.html', patterns=patterns, message="")

@app.route('/overdue-tracker')
def overdue_tracker():
    """Overdue Tracker - Numbers that haven't appeared recently"""
    df = load_csv_data()
    if df.empty:
        return render_template('overdue_tracker.html', overdue=[], message="No data available")
    
    # Get all numbers from last 100 draws
    recent_nums = set()
    for col in ['1st_real', '2nd_real', '3rd_real']:
        recent_nums.update([n for n in df[col].tail(100).astype(str) if n.isdigit() and len(n) == 4])
    
    # Get all numbers from history
    all_nums = set()
    for col in ['1st_real', '2nd_real', '3rd_real']:
        all_nums.update([n for n in df[col].astype(str) if n.isdigit() and len(n) == 4])
    
    # Find overdue (appeared before but not recently)
    overdue = list(all_nums - recent_nums)[:100]
    
    # Calculate last seen date for each
    overdue_data = []
    for num in overdue:
        last_seen = None
        for _, row in df.iterrows():
            if num in [str(row['1st_real']), str(row['2nd_real']), str(row['3rd_real'])]:
                last_seen = row['date_parsed'].date()
        if last_seen:
            days_ago = (datetime.now().date() - last_seen).days
            overdue_data.append({'number': num, 'last_seen': last_seen, 'days_ago': days_ago})
    
    overdue_data.sort(key=lambda x: x['days_ago'], reverse=True)
    
    return render_template('overdue_tracker.html', overdue=overdue_data[:50], message="")

@app.route('/box-play')
def box_play():
    """Box Play - Generate permutations"""
    df = load_csv_data()
    if df.empty:
        return render_template('box_play.html', suggestions=[], message="No data available")
    ...
    for num, count in freq:
        ...
        perms = list(set([''.join(p) for p in permutations(num)]))
        suggestions.append({
            'number': num,
            'frequency': count,
            'permutations': perms[:24],  # Max 24 permutations
            'total_combinations': len(perms),
            'total_cost': len(perms)  # RM1 per number
        })
    
    return render_template('box_play.html', suggestions=suggestions, message="")

@app.route('/lstm-predictor')
def lstm_predictor():
    """LSTM Predictor - Deep learning style prediction"""
    df = load_csv_data()
    if df.empty:
        return render_template('lstm_predictor.html', predictions=[], message="No data available")
    
    # Use ML predictor as LSTM alternative
    predictions = ml_predictor(df, lookback=500)[:10]
    
    return render_template('lstm_predictor.html', predictions=predictions, message="")

@app.route('/ensemble-predictor')
def ensemble_predictor():
    """Ensemble Predictor - Combine all methods"""
    return redirect('/ultimate-predictor')

@app.route('/odds-calculator')
def odds_calculator():
    """Odds Calculator - Calculate winning odds"""
    return render_template('odds_calculator.html', 
                         odds={
                             '1st_prize': '1 in 10,000',
                             '2nd_prize': '1 in 10,000',
                             '3rd_prize': '1 in 10,000',
                             'special': '1 in 1,000',
                             'consolation': '1 in 1,000',
                             'box_play': '1 in 417 (24 permutations)'
                         },
                         message="")
