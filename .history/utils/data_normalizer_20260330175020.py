...
def normalize_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Fix MultiIndex CSV (date + provider as index)
    if isinstance(df.index, pd.MultiIndex):
        df = df.reset_index()
        logger.info(f"Reset MultiIndex: {len(df)} rows")
    
    # Ensure columns exist, use defaults if missing
    expected_columns = ['date', 'provider', 'draw_info', '1st', '2nd', '3rd', 'special', 'consolation']
    if list(df.columns) != expected_columns:
        logger.warning(f"Unexpected columns: {list(df.columns)}, expected: {expected_columns}")
        # Rename if possible
        if len(df.columns) == len(expected_columns):
            df.columns = expected_columns
        else:
            raise ValueError(f"Column mismatch: got {len(df.columns)}, expected {len(expected_columns)}")
    
    # Parse date from 'date' column
    df['date_parsed'] = pd.to_datetime(df['date'], errors='coerce')
    
    # Get provider from 'provider' column (URL) - this is the TRUE provider
    df['provider_key'] = df['provider'].apply(DataNormalizer.normalize_provider)
    
    # 🎯 CLASSIFY: Add game_type based on draw_info
    df['game_type'] = df['draw_info'].astype(str).apply(lambda x: 
        '4d' if '4D' in x.upper() and '5D' not in x.upper() and '6D' not in x.upper() else
        '5d6d' if '5D' in x.upper() or '6D' in x.upper() else
        'jackpot' if x.upper().strip() == 'JACKPOT' else
        'mixed'
    )
    
    # 🎯 FILTER: Skip non-4D lottery types (5D, 6D, Lotto, etc.)
    df['lottery_type'] = df['draw_info'].astype(str).str.lower()
    ...
    for idx, row in df.iterrows():
        # '3rd': prize_text (1st/2nd/3rd prizes)
        prize_text = str(row['3rd']) if '3rd' in df.columns else ''
        # 'special': special
        special_text = str(row['special']) if 'special' in df.columns else ''
        # 'consolation': consolation
        consolation_text = str(row['consolation']) if 'consolation' in df.columns else ''
    ...
    # Add raw_row_type for debugging
    df['raw_row_type'] = df['draw_info']
