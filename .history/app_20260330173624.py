...
def master_analyzer():
    ...
    all_numbers = [n for col in ['1st_real', '2nd_real', '3rd_real'] for n in df[col].dropna().astype(str) if n.isdigit() and len(n) == 4]
...
def export_statistics():
    df = load_csv_data()
    all_nums = [n for col in ['1st_real', '2nd_real', '3rd_real'] for n in df[col].dropna().astype(str) if n.isdigit() and len(n) == 4]
    freq = Counter(all_nums)
...
def power_dashboard():
    ...
    try:
        ...
        for col in ['1st_real', '2nd_real', '3rd_real']:
            if col in df.columns:
                all_numbers.extend([n for n in df[col].dropna().astype(str) if len(n) == 4 and n.isdigit()])
...
def decision_helper():
    ...
    if not adv and not smart and not ml:
        ...
        all_nums = []
        for col in ['1st_real', '2nd_real', '3rd_real']:
            all_nums.extend([n for n in df[col].tail(100).dropna().astype(str) if n.isdigit() and len(n) == 4])
...
def learning_dashboard():
    ...
    # Recent predictions
    recent_predictions = completed.tail(10).to_dict(orient='records') if not completed.empty else []
