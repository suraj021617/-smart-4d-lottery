...
def load_csv_data():
    ...
    # Fill empty values
    df['1st_real'] = df['1st_real'].fillna('')
    df['2nd_real'] = df['2nd_real'].fillna('')
    df['3rd_real'] = df['3rd_real'].fillna('')
...
if __name__ == '__main__':
    app.run(debug=True, host='0.0.0.0', port=5000)get('draw_date'),
        provider=data.get('provider')
    )
...
def past_results():
    ...
    # Process results to extract real 4D numbers
    results = []
    for _, row in filtered.iterrows():
        result = row.to_dict()
        # Extract real 4D numbers from prize text
        ...
        result['second_prize'] = second_match.group(1) if second_match else '----'
        result['third_prize'] = third_match.group(1) if third_match else '----'
